...
export default async function build(options: BuildOptions) {
  ...
  for (let folder of folders) {
    ...
    for (const file of files) {
      ...
      for (const step of folder.build.filePipeline) {
        ...
        prev_res = await step(config, getFileInfo(file, path, project_dir, resolveOutDir(config, ':output:')), prev_res);
      }
    }
  ...
  for (const step of config.build.slashPipeline) {
    ...
    prev = step(config, getFileInfo(rootFile, project_dir, project_dir, resolveOutDir(config, ':output:')), prev);
  }
